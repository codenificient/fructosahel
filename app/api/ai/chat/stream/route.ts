...
import { streamChat } from "@/lib/ai-agents/client";
import { getAgentConfig } from "@/lib/ai-agents/config";
import type { AgentType } from "@/types";

// Mock streaming for development without API key
async function* mockStreamChat(text: string): AsyncGenerator<string> {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Check for API key - use mock streaming if not available
    if (!process.env.ANTHROPIC_API_KEY) {
      ...
      const stream = new ReadableStream({
        async start(controller) {
          for await (const chunk of mockStreamChat(mockResponse)) {
    ...
    // Real streaming with Claude API
    const stream = new ReadableStream({
      async start(controller) {
        try {
